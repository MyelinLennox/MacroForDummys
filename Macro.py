...
if __name__ == '__main__':
    ...
    while True:
        if keyboard.is_pressed(record_key):
            if record_process is None or not record_process.is_alive():
                print("Starting macro recording...")
                record_process = multiprocessing.Process(target=record_macro)
                record_process.start()
                record_process.join()
                print("Recording finished.")
        elif keyboard.is_pressed(activate_key):
